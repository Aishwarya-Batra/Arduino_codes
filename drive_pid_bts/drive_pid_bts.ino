...
void rpm_calculate() {
   ...
   if (joystick1.available()) {
    ...
    if (motorOutput >= 0) {
    analogWrite(LPWM,0);
    analogWrite(RPWM,(int)motorOutput);
    } 
    else {
    analogWrite(LPWM,(int)fabs(motorOutput));
    analogWrite(RPWM,0);
    }
...
void setup() {
    ...
    analogWriteFrequency(EN_PIN,9000);

    rpm_time.begin(rpm_calculate, 75000); // Call rpm_calculate every 100ms
